		...
		<form name="detailForm" action="/listProduct.do?menu=${param.menu}"
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0"
				...
				<td align="right">
				 ...
				 <input type="text" name="searchKeyword" value="${earch.searchKeyword}" class="ct_input_g"
		...
		<tr class="ct_list_pop">
			<td align="center">${ i }</td>
					<td></td>
								<c:if test= "${param.menu eq 'manage'}">
								<td align="left"><a
						href="/updateProductView.do?prodNo=${prod.prodNo}">${prod.prodName}</a></td>
								</c:if>
								<c:if test= "${param.menu eq 'search' }">
										<td align="left"><a
						href="/getProduct.do?prodNo=${prod.prodNo}">${prod.prodName}</a></td>
								</c:if>
